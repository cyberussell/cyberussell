...
## Known Issues
- **Migration 031 not yet applied to the live TMS Supabase project** — until Russell runs it, submitting any correction that touches Household Members (or any correction at all, since the recommend-write always includes all four/five `correction_recommended_*` columns together) will fail in production with a missing-column error. Same standing pattern as every other TMS migration this session:
  ```sql
  alter table public.territory_records add column if not exists correction_recommended_household_members int;
  ```
- Live-verified via a temporary scratch route (removed before finishing): the redesigned card matches the reference mockup closely (screenshot-compared side by side), the back arrow fires `onBack`, Household Members correctly prefills from the record and the Send button stays disabled until a real field changes, and the live Plus Code format validation shows/hides its error text correctly. Not verified: the actual database round-trip (no live Supabase credentials in this sandbox — standing limitation) and the Admin's Flagged for Correction page's new Household Members line (code-reviewed only, same reasoning as the existing Plus Code/Section/Block lines it sits next to).

## Next Recommended Task
Russell runs migration 031, then live-verifies: recommending a Household Members correction from a real assigned record, confirming it shows correctly on the Admin's Flagged for Correction page, and that "Apply Correction" actually updates the record's real Household Members count. Also worth a quick call on the status-coloring judgment call above — confirm the icon-badge-only tinting is right, or say if the card should go fully white with no status color at all.
