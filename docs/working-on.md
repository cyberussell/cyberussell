...
Russell asked for the draft to "write itself" instead of appearing instantly, and for the send step to visibly animate through email → calendar with simple delays, using neon-glowing automation icons. Clarified via AskUserQuestion first: "remotion-like" meant the cinematic, precisely-sequenced *feel* of Remotion, built with the GSAP tooling already powering this page (not the actual `remotion` package, which is a video-rendering library and a poor fit for a live widget reacting to real async API calls) — and "neon light automation icons" meant one glowing Lucide icon per pipeline stage (not a single icon traveling along the pipeline).

Changes, `/automation-demo` only:
- `PipelineStageCard.tsx`: added an optional `icon` prop (`LucideIcon`). When set, the stage's status badge renders that icon in a neon circular badge instead of the step-number/✓/…/! glyph — dim when idle, pulsing (`motion-safe:animate-[neon-pulse_...]`, CSS-only, respects `prefers-reduced-motion` via the `motion-safe:` variant like the rest of this page) with a strong glow when active, steady soft glow when done, dim red glow on error.
- `LiveDemo.tsx` (draft — typewriter): a GSAP tween types the subject then the email body into state character-by-character (duration scales with text length, capped), with a blinking caret (`demo-caret-blink` keyframe) at the point of typing. Under `prefers-reduced-motion` the full text is set instantly, matching how the rest of the page already handles reduced motion. "Send for real" is disabled until typing finishes. The proposed-slot/log-summary lines are held back until typing completes.
- `LiveDemo.tsx` (send — cascading reveal): the real `/api/automation-demo/run` call is unchanged (still one request, all three actions happen server-side together) — the new `stageReveal` state (0→3) sequences how the *result* is revealed client-side after it comes back: Send lights up and resolves first, then Schedule lights up and resolves, then Log, ~900ms apart (`STAGE_REVEAL_DELAY_MS`), rather than all three flipping to done at once. Each stage's "View event"/"View file" link is gated to only appear once that stage has been revealed — caught and fixed a bug during verification where the real links (already present in the API response) were leaking into the UI early, before their card had visually resolved. A `runIdRef` guards against a stale sequence finishing after "Start over"/"Disconnect".
- `HowItWorks.tsx`: passed the same 4 icons (`Sparkles`/`Mail`/`CalendarClock`/`FolderOpen`) into its own `PipelineStageCard` usage for visual consistency with the live demo (steady done-glow, matches the "shared visual language" already established for this component).
- `globals.css`: added `neon-pulse` and `demo-caret-blink` keyframes to the existing "/automation-demo page decorative keyframes" block.

Verified with `npx tsc --noEmit` (clean) and Playwright against a local dev server. Since no Google OAuth client is configured in this environment, exercised the new animation by intercepting `/api/automation-demo/{status,draft,run}` with mocked responses (status→configured+connected, draft→a sample lead reply, run→all-ok result) to drive the real UI through connect→draft→send with real timers — confirmed the typewriter caret mid-type, the draft settling before Send/Schedule/Log begin, each stage lighting up and resolving in sequence with its link appearing only once resolved, and the `HowItWorks` section's icons rendering with steady glow. No mock was left in the shipped code — those routes are untouched; only the client interception in the local test script simulated a connected session.
