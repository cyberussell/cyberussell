...
**Latest batch (see checkpoint `appointment-system-ux-polish-v1.md`), all verified live except the Messenger piece (verified via tsc + code parity only):**
...
- **Reference code now visible on dashboard appointment cards**: `dashboard/appointments/page.tsx` week-view list now shows a `#XXXXXX` chip next to the status badge (field already existed since migration 008, just wasn't selected/rendered here). Confirmed old pre-migration rows correctly show no chip.
- **Messenger booking now asks which staff member**, mirroring the web `BookingWidget`'s existing behavior: slots are deduped by time first, and only when 2+ staff are actually free at the chosen time does the bot ask "Sino po ang gusto niyong provider?" before finalizing. Previously it silently auto-picked whichever staff's slot happened to be in the list. Added `'choosing_staff'` to `FlowStep` in `types.ts`. **Not yet verified against a live Messenger conversation** — only via type-check and logic parity with the already-tested web flow.
