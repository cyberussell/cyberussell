...
// Fixed-window counter backed by the `rate_limits` table (migration 016).
// Fails open — a missing table (migration not yet run) or a transient DB
// error never blocks a legitimate request; rate limiting degrading is far
// preferable to login/signup breaking because of it. Ported from the
// Appointment System's identical helper.
