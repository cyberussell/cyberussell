          ...
          <div className="flex flex-wrap gap-4 mt-4">
            {HERO_STATS.map(({ value, label, color, bg, border }) => (
              <div key={value} className={`flex flex-col ${bg} border ${border} rounded-lg p-4 min-w-[140px] flex-1`}>
                ...
                <span className="font-[family-name:var(--font-inter)] text-[12px] text-white/65">{label}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="flex flex-col gap-6">
            {PAIN_STAT_CARDS.map(({ stat, title, desc, source }) => (
              ...
              >
                ...
                <p className="font-[family-name:var(--font-inter)] text-[16px] text-white/70 mb-6 leading-[1.8]">
                  {desc}
                </p>
