...
const ICONS = { ShieldCheck, HeartPulse, Clock, Award };

export default function WhyUs() {
  return (
    <section id="why-us" className="px-6 md:px-10 py-20 md:py-28 bg-[#0B1220]">
      <div className="max-w-6xl mx-auto">
        <motion.div variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }} className="max-w-[560px] mb-14">
          ...
          <h2 className="font-sans text-[28px] md:text-[38px] font-extrabold text-white leading-tight tracking-tight">
            Dental care that actually feels comfortable.
          </h2>
        </motion.div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-5">
          {FEATURES.map((feature, i) => {
            ...
            return (
              ...
              >
                <div className="w-11 h-11 rounded-xl bg-[#0D9488]/20 flex items-center justify-center">
                  <Icon size={20} className="text-[#5EEAD4]" />
                </div>
                <h3 className="font-sans text-[15.5px] font-bold text-white leading-snug">{feature.title}</h3>
                <p className="font-[family-name:var(--font-inter)] text-[13px] text-white/50 leading-[1.6]">
                  {feature.desc}
                </p>
              </motion.div>
            );
