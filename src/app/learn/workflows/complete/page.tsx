...
const BENEFITS = [
  { icon: Medal, color: "#F59E0B", text: "Earn your Workflow Bida Badge" },
  { icon: ScrollText, color: "#818CF8", text: "Receive your AI Workflows Certificate" },
  { icon: Save, color: "#34D399", text: "Save your learning progress forever" },
  { icon: Rocket, color: "#60A5FA", text: "Unlock Pillar 5" },
...
export default function WorkflowsCompletePage() {
  ...
  useEffect(() => {
    fetch("/api/learner/auth").then((r) => r.json()).then((d) => {
      if (d.learner) window.location.href = "/learn/workflows/assessment";
    });
    ...
    const tokenClient = window.google.accounts.oauth2.initTokenClient({
      ...
      callback: async (response: { access_token?: string; error?: string }) => {
        ...
        try {
          ...
          if (res.ok) {
            window.location.href = "/learn/workflows/assessment";
          } else {
        ...
        <div className="w-full max-w-[520px]">
          ...
          <div className="text-center mb-8">
            ...
            <p className="font-[family-name:var(--font-inter)] text-[16px] text-white/60 leading-relaxed">
              ...
              <span className="text-white font-semibold">Pillar 4: AI Workflows.</span>
            </p>
          </div>
          ...
          <div className="bg-[#14141e] border border-white/[0.08] rounded-2xl p-6 mb-6">
            <p className="font-[family-name:var(--font-inter)] text-[14px] text-white/70 leading-relaxed mb-5 text-center">
              You&rsquo;re one step away from earning your next Cyberussell achievement.
              <br />
              <span className="text-white/90 font-medium">Take the final assessment to earn your Workflow Bida Badge and personalized certificate.</span>
            </p>
            ...
            {mode === "signin" && (
              <form onSubmit={handleEmailRegister} className="space-y-3">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full py-3.5 rounded-xl bg-[#F59E0B] text-[#0a0a12] font-[family-name:var(--font-inter)] text-[14px] font-bold hover:bg-[#F59E0B]/90 transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
                >
