...
export default function WorkflowsAssessmentPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/learner/auth").then((r) => r.json()).then((d) => {
      if (!d.learner) { window.location.href = "/learn/workflows/complete"; return; }
      setLearnerName(d.learner.name);
      // If already passed, redirect to congratulations
      if (d.learner.completed_pillars?.includes("workflows")) {
        window.location.href = "/learn/workflows/congratulations";
      }
    });
    // Load questions
    fetch("/api/learner/workflows-assessment").then((r) => r.json()).then((d) => {
  ...
  async function handleSubmit() {
    ...
    const res = await fetch("/api/learner/workflows-assessment", {
      method: "POST",
    ...
    if (res.ok) {
      setResult(data);
      if (data.passed) {
        setTimeout(() => { window.location.href = "/learn/workflows/congratulations"; }, 2000);
      }
    } else {
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="mb-10">
            <div className="inline-flex items-center gap-2 bg-[#F59E0B]/10 border border-[#F59E0B]/20 rounded-full px-4 py-1.5 mb-4">
              <span className="text-[#F59E0B] text-[11px] font-bold uppercase tracking-[2px] font-[family-name:var(--font-inter)]">Final Assessment</span>
            </div>
            <h1 className="font-sans text-[28px] md:text-[36px] font-bold text-white mb-2">AI Workflows</h1>
            ...
            <div className="mt-5">
              ...
              <div className="h-1.5 bg-white/[0.06] rounded-full overflow-hidden">
                <div className="h-full bg-[#F59E0B] rounded-full transition-all duration-300" style={{ width: `${(answered / total) * 100}%` }} />
              </div>
            </div>
          </div>
          ...
          {!result && (
            <div className="space-y-6">
                    ...
                    {q.options.map((opt, oi) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full flex items-center gap-3 p-4 rounded-xl border text-left transition-all ${selected ? "border-[#F59E0B]/60 bg-[#F59E0B]/[0.08]" : "border-white/[0.06] hover:border-white/[0.12] hover:bg-white/[0.02]"}`}
                        >
                          <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 ${selected ? "border-[#F59E0B] bg-[#F59E0B]" : "border-white/20"}`}>
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-4 rounded-2xl bg-[#F59E0B] text-[#0a0a12] font-[family-name:var(--font-inter)] text-[15px] font-bold hover:bg-[#F59E0B]/90 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
              >
            ...
            <div className="bg-[#14141e] border border-white/[0.06] rounded-2xl p-8 text-center">
              {result.passed ? (
                <>
                  ...
                  <Loader2 size={20} className="text-[#F59E0B] animate-spin mx-auto mt-4" />
                </>
              ) : (
                <>
                  ...
                  <div className="flex gap-3 justify-center">
                    <a href="/learn/workflows" className="px-5 py-2.5 rounded-xl border border-white/[0.10] font-[family-name:var(--font-inter)] text-[13px] text-white/60 hover:text-white/80 transition-colors">Review Guides</a>
                    <button onClick={() => { setResult(null); setAnswers({}); }} className="px-5 py-2.5 rounded-xl bg-[#F59E0B] text-[#0a0a12] font-[family-name:var(--font-inter)] text-[13px] font-bold hover:bg-[#F59E0B]/90 transition-colors">Try Again</button>
                  </div>
                </>
              )}
            </div>
