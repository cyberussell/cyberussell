import type { Metadata } from "next";
import Navbar from "@/components/Navbar";
import Footer from "@/components/Footer";

export const metadata: Metadata = {
  title: "Career Guides — Free Resources for Filipinos | Cyberussell",
  description:
    "Free career guides for Filipinos — how to choose a career path, upskill, build an online income, and grow from zero.",
  alternates: { canonical: "https://www.cyberussell.com/learn/career-guides" },
};

export default function LearnCareerGuidesPage() {
  return (
    <>
      <Navbar />
      <main className="min-h-screen bg-[#0F0F1A] px-6 md:px-10 py-16 max-w-4xl mx-auto">
        <nav className="flex items-center gap-2 text-[12px] font-[family-name:var(--font-inter)] text-white/35 mb-8">
          <a href="/" className="hover:text-white transition-colors">Home</a>
          <span>/</span>
          <span className="text-white/60">Learn</span>
          <span>/</span>
          <span className="text-white/60">Career Guides</span>
        </nav>
        <div className="inline-block bg-[#E8373A]/10 border border-[#E8373A]/20 rounded-full px-4 py-1.5 mb-5">
          <span className="text-[#E8373A] text-[11px] font-bold uppercase tracking-[2px] font-[family-name:var(--font-inter)]">
            📚 Learn
          </span>
        </div>
        <h1 className="font-sans text-[32px] md:text-[44px] font-bold text-white mb-4 leading-tight">
          Career Guides
        </h1>
        <p className="font-[family-name:var(--font-inter)] text-[16px] text-white/60 leading-[1.8] mb-10 max-w-2xl">
          AI-generated career roadmaps and guides built for Filipinos — skill paths, income
          projections, platform recommendations, and step-by-step blueprints for each career.
        </p>
        <div className="bg-[#18181F] border border-white/[0.08] rounded-2xl p-8 text-center">
          <p className="font-[family-name:var(--font-inter)] text-[15px] text-white/50 mb-4">
            Explore the full Career Blueprints library:
          </p>
          <a
            href="/careers"
            className="inline-block bg-[#E8373A] text-white font-bold font-[family-name:var(--font-inter)] text-[14px] px-6 py-3 rounded-lg hover:opacity-90 transition-all"
          >
            View Career Blueprints →
          </a>
        </div>
      </main>
      <Footer />
    </>
  );
}
