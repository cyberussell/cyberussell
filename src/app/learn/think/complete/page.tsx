...
const BENEFITS = [
  { icon: Medal, color: "#A78BFA", text: "Earn your Thinking Bida Badge" },
  ...
  { icon: Rocket, color: "#60A5FA", text: "Unlock Pillar 3" },
...
export default function ThinkCompletePage() {
  ...
  useEffect(() => {
    fetch("/api/learner/auth").then((r) => r.json()).then((d) => {
      if (d.learner) window.location.href = "/learn/think/assessment";
    });
    ...
    const tokenClient = window.google.accounts.oauth2.initTokenClient({
      ...
      callback: async (response: { access_token?: string; error?: string }) => {
        ...
        try {
          ...
          if (res.ok) {
            window.location.href = "/learn/think/assessment";
          } else {
