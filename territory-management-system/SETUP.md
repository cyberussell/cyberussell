...
assigning, and tracking territory work during field ministry, at `/territory-management-system`.
It runs on its **own Supabase project** (separate from the main site's, the Appointment
...
- `src/app/territory-management-system/` — routes (login + the Administrator dashboard)
...
4. The admin can now sign in at `/territory-management-system/login`.
...
```
They sign in at the same `/territory-management-system/login` — the app redirects admins to
